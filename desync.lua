...
local function update()
    if (np.currLevelNum ~= gNetworkPlayers[network_local_index_from_global(0)].currLevelNum or not np.currAreaSyncValid) and (gGlobalSyncTable.roundState == ROUND_ACTIVE or gGlobalSyncTable.roundState == ROUND_WAIT or gGlobalSyncTable.roundState == ROUND_HOT_POTATO_INTERMISSION) and desyncTimer > 0 then
